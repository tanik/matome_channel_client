import * as actions from '../../../src/js/actions/user/board'
...
describe('actions', () => {
  ['boards', 'favorite_boards', 'histories'].forEach( (item_type) => {
    it(`should create an action to get my ${item_type} async success`, () => {
      ...
      const expectedActions = [{
        type: types.GET_MY_BOARDS,
        boards: resp_data.boards,
        pagination: resp_data.pagination,
      }]
      const store = mockStore()
      return store.dispatch(actions.getMyBoardsAsync(item_type, page, per)).then(() => {
